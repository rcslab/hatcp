    ...
    uint32_t rso_addr, uint16_t rso_port)
{
	...
	error = create_socket(&fd);
	if (error) 
		return (error);

	/* [REPLICA] Using somig to init the socket first. */
	if (somig_mode == NETLB_SERVER_SMG_REPLICA) {
	...
	error = bind(fd, (struct sockaddr *)&address, sizeof(address));
	if (error < 0) {
		SYSERR(errno, "Failed to bind.\n");
		exit(0);
	}
	...
	/* [PRIMARY] Init somig */
	if (somig_mode == NETLB_SERVER_SMG_PRIMARY) {
