...
/*
    ...
    */
	...
	if (bt == NULL) {
		net_mem_init_size = size; 
	} else
		*bt = size - net_mem_init_size;
...
cpu_util(int *syscpu, int *usrcpu, struct rusage *rlast)
{
    ...
    /*
    printf("rt time sec %u rlast time sec %u\n", rtemp.ru_utime.tv_sec, rlast->ru_utime.tv_sec);
    printf("rt time usec %u rlast time usec %u\n", rtemp.ru_utime.tv_usec, rlast->ru_utime.tv_usec);
    printf("rt time sec %u rlast time sec %u\n", rtemp.ru_stime.tv_sec, rlast->ru_stime.tv_sec);
    printf("rt time usec %u rlast time usec %u\n", rtemp.ru_stime.tv_usec, rlast->ru_stime.tv_usec);
    */
