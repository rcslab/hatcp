...
int 
init_socket_socks(int *ifd, int mode, uint32_t addr, uint16_t port, 
    ...
    uint32_t rso_addr, uint16_t rso_port)
{
	...
	if (mode == SOCKS_SERVER) {
		/* [REPLICA] Using somig to init the socket first. */
		if (somig_mode == SOCKS_SERVER_SMG_REPLICA) {
		...
		error = bind(fd, (struct sockaddr *)&address,
		    sizeof(address));
		if (error < 0) {
		...
		/* [PRIMARY] Init somig */
		if (somig_mode == SOCKS_SERVER_SMG_PRIMARY) {
...
int
init_socks_socket(char *rawaddr, uint16_t port, int type, uint8_t *rep)
{
	...
	if (type == SOCKS_ATYP_DOMAINNAME) {
		memcpy(&hostaddr, rawaddr, strlen(rawaddr));
		hostaddr[strlen(rawaddr)] = '\0';
		if ((host = gethostbyname(hostaddr)) == NULL) {
			DBG("REQ - Cannot resolve hostname.");
			return (0);
		}
		nettype = host->h_addrtype;
		addr.sin_addr.s_addr = *(in_addr_t *)host->h_addr;
	}
	else if (type == SOCKS_ATYP_IPV4 || type == SOCKS_ATYP_IPV6) {
		nettype = type == SOCKS_ATYP_IPV4 ? AF_INET : AF_INET6;
		if (inet_pton(AF_INET, rawaddr, &addr.sin_addr) <= 0) {
			APPERR("Invalid SOCKS connect address.\n");
			return (0);
		}
	} else
		return (0);

	addr.sin_family = nettype;
	addr.sin_port = htons(port);

	fd = socket(AF_INET, SOCK_STREAM, 0);
	if (fd < 0) {
		APPERR("Failed to create SOCKS socket.\n");
		return (0);
	}
	if (connect(fd, (struct sockaddr *)&addr, sizeof(struct sockaddr)) != 0) {
		APPERR("Failed to connect to SOCKS target address.\n");
		return (0);
	}
	
	//TODO set rep, refer RFC
	*rep = 0;
	return fd;
}
