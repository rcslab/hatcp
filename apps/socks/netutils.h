 ...
 *
 * In summary, the name ended with "_socks" is for socks app use. (This netutil
 *  file is shared by both socks and wanacc)
 *
 * init_socket is used for general propose socket init.
 * init_socket_socks is used for socks specific socket init(see params for difference)
 * init_socks_socket is used for socket on remote server to connect to user
 *	requested server.
 */
int init_socket(int *);
int init_socket_socks(int *, int, uint32_t, uint16_t, int, uint32_t, uint16_t, 
    uint32_t, uint16_t);
int accept_socket(int);
void listen_socket(int);
int bind_socket(int, uint32_t, uint16_t);
int connect_socket(int, uint32_t, uint16_t);
int init_socks_socket(char *rawaddr, uint16_t port, int type, uint8_t *rep);
...
int get_sock_avail_bytes(int fd);
