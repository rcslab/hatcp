...
#else
static cJSON *JSON_readN(int fd, uint32_t len);
#endif
#ifndef IPERF_LITE
...
int IPERF_LITE_START_SIGNAL = 0xdeadf00d;
#endif
...
#if defined(IPERF_LITE)
        {"reverselisten", no_argument, NULL, 'r'},
        {"rlhostname", required_argument, NULL, 'E'},
#endif
...
#if defined(IPERF_LITE)
    while ((flag = getopt_long(argc, argv, "p:f:i:D1VJvsc:ub:t:n:k:l:P:Rrw:B:M:E:N46S:L:ZO:F:A:T:C:dI:hX:", longopts, NULL)) != -1) {
#else
...
#if defined(IPERF_LITE)
	    case 'r':
		test->reverse_listen=1;
		break;
	    case 'E':
		test->rl_hostname = strdup(optarg);
		break;
#endif
...
#ifdef IPERF_LITE
    if (len) { 
	j = JSON_readN(test->ctrl_sck, len);
    } else {
	len = 0;
	r = Nread(test->ctrl_sck, (char*) &len, sizeof(len), Ptcp);
	return (r>0? 0: -1);
    }
#else
...
{
    return (get_results_N(test, -1));
}

int
get_results_N(struct iperf_test *test, uint32_t len)
#else
static int
get_results(struct iperf_test *test)
#endif
...
static cJSON *
#ifdef IPERF_LITE
JSON_readN(int fd, uint32_t len)
#else
JSON_read(int fd)
#endif
{
	...
	if (str != NULL) {
#ifdef IPERF_LITE
	    rc = Nread_with_flag(fd, str, hsize, Ptcp, READ_FULLSIZE);
	    if (rc == 0) {
		free(str);
		return (NULL);
	    }
