...
iperf_run_client(struct iperf_test * test)
{
    ...
    printf("on_connect..\n");
#endif
    /* We manipulate all the param exchange steps */
    if (test->on_connect)
	test->on_connect(test);

#if (IPERF_LITE_DEBUG)
    printf("iperf_create_stream..\n");
#endif
    test->num_streams = 1;
    if (iperf_create_streams(test, test->mode) < 0)
	return -1;

#if (IPERF_LITE_DEBUG)
    printf("Waiting for start signals..\n");
#endif
    int il_sig = 0;
    while (1) {
	int r = 0;
	...
	SLIST_FOREACH(sp, &test->streams, streams) {
		...
		if ((r = Nread(sp->socket, (char *)&(il_sig), sizeof(int), Ptcp)) < 0) {
    ...
    if (il_sig != IPERF_LITE_START_SIGNAL) {
	printf("Bad signal. Exprecting %d, %d received.\n", IPERF_LITE_START_SIGNAL, il_sig);
	exit(0);
    }

#if (IPERF_LITE_DEBUG)
    printf("Signal received. Starting in 1 seconds..\n");
#endif
    sleep(1);

#if (IPERF_LITE_DEBUG)
    /* Send test duration to server */
    printf("Sending params..\n");
#endif
    SLIST_FOREACH(sp, &test->streams, streams) {
	if (Nwrite(sp->socket, (char *)&(test->duration), sizeof(int), Ptcp) < 0) {
    ...
    printf("iperf_init_test..\n");
#endif
    if (iperf_init_test(test) < 0)
	return -1;
#if (IPERF_LITE_DEBUG)
    printf("create_client_timers..\n");
#endif
    if (create_client_timers(test) < 0)
	return -1;
#if (IPERF_LITE_DEBUG)
    printf("create_client_omit_timer..\n");
#endif
    if (create_client_omit_timer(test) < 0)
	return -1;
#if (IPERF_LITE_DEBUG)
    printf("iperf_create_send_timers..\n");
#endif
    if (test->mode)
	if (iperf_create_send_timers(test) < 0)
	    return -1;
