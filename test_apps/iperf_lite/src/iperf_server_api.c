...
#ifdef IPERF_LITE
#define PEER_INIT	0
#define PEER_LISTEN	1
#define PEER_ACCEPTED	2
#define PEER_RUNNING	3
#define PEER_TEST_END1	    100
#define PEER_TEST_END2	    101
#define PEER_DONE	    102

#define IPERF_LITE_ARG_MAGIC 0xf00d2bad
struct iperf_lite_arg {
	uint32_t magic;
	uint32_t flag;
	struct iperf_test *test;
};

static int iperf_server_loop(void *arg); /* arg = iperf_lite_arg */

#define LOOP_ONCE   return (0)
#define LOOP_ERROR  return (-1)
#endif
...
#ifdef IPERF_LITE
int
iperf_accept_rl(int fd, struct iperf_test *test)
{
    int s;
...
server_reporter_timer_proc(TimerClientData client_data, struct iperf_time *nowP)
{
    ...
    struct iperf_time temp_time, now;
    
    sp = SLIST_FIRST(&test->streams); 
    irp = TAILQ_FIRST(&sp->result->interval_results);
    if (irp) {
	iperf_time_now(&now);
	iperf_time_diff(&irp->interval_start_time, &now, &temp_time);
	if (temp_time.secs >= test->duration) {
		test->done = 1;
	}
    }
#endif
...
#ifdef IPERF_LITE
static int 
iperf_exchange_parameters_lite(struct iperf_test *test, int stage) 
{
    int s;
    int32_t err;

    if (test->role == 'c') {
	printf("Shouldn't be the client..\n");
	exit(0);
	...
	printf("Failed to listen on prot_listener\n");
	if (iperf_set_send_state(test, SERVER_ERROR) != 0)
            return -1;
        err = htonl(i_errno);
        if (Nwrite(test->ctrl_sck, (char*) &err, sizeof(err), Ptcp) < 0) {
...
iperf_loop_accept(int fd) 
{
    ...
    len = sizeof(addr);
    if ((so = accept(fd, (struct sockaddr *)&addr, &len)) < 0) {
	i_errno = IEACCEPT;
	return (-1);
    }
...
iperf_run_server(struct iperf_test *test)
{
...
#if defined(IPERF_LITE)
    if (test->reverse_listen) {
	// connect rl_fd to the remote peer
	rl_fd = netdial(test->settings->domain, 
			Ptcp, test->bind_address, test->bind_dev, 0,
			test->rl_hostname, test->rl_port,
	...
	if (result > 0) {
		    ...
		    goto rl_fd_cont;
rl_fd_init:
		    //save rl_fd to stream
		    if (iperf_accept_rl(rl_fd, test) < 0) {
			cleanup_server(test);
			return -1;
		    }    
		    rl_fd = -1;
rl_fd_cont:
		...
		if (1) {
		    ...
		    SLIST_FOREACH(sp, &test->streams, streams) {
			if (Nwrite(sp->socket, (char *)&IPERF_LITE_START_SIGNAL, sizeof(int), Ptcp) < 0) {
			    i_errno = IESENDPARAMS;
			    return -1;
			}
		    }

		    printf("Reading client params..\n");
		    /* Read the test duration from the client */
		    while (1) {
			int sresult = select(test->max_fd + 1, &test->read_set, NULL, NULL, timeout);
			...
			SLIST_FOREACH(sp, &test->streams, streams) {
			    if (FD_ISSET(sp->socket, &test->read_set) && !sp->sender) {
				if ((r = Nread(sp->socket, (char *)&(test->duration), sizeof(int), Ptcp)) < 0) {
				    printf("Cannot read duration from client.\n");
				    exit(0);
				}
				FD_CLR(sp->socket, &read_set);
			    }
			}
