...
{
    return (Nread_with_flag(fd, buf, count, prot, 0));
}
#endif

int
#ifdef IPERF_LITE
Nread_with_flag(int fd, char *buf, size_t count, int prot, int flag)
#else
Nread(int fd, char *buf, size_t count, int prot)
#endif
{
...
#ifdef IPERF_LITE
    if (flag & READ_FULLSIZE) {
	r = recv(fd, buf, nleft, MSG_PEEK);
	if (r < nleft) 
	    return (0);
    }
#endif
