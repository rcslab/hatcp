...
//#endif
    ...
    if (test->json_output)
//#ifdef SOMIGRATION
	cJSON_AddItemToObject(test->json_start, "timestamp", iperf_json_printf("time: %s  timesecs: %d  accutime_sec: %d accutime_usec: %d", now_str, (int64_t) now_secs, tn.tv_sec, tn.tv_usec));
//#else
...
iperf_parse_arguments(struct iperf_test *test, int argc, char **argv)
{
    ...
    {
#ifdef SOMIGRATION
	{"somig-migration", required_argument, NULL, 'K'},
	{"somig-execute-kernel-for-failover", no_argument, NULL, 'E'},
	{"somig-unified-traffic-pattern", no_argument, NULL, 'U'},
	{"somig-mode", required_argument, NULL, 'm'},
	{"somig-primary-addr", required_argument, NULL, 'a'},
	{"somig-replica-addr", required_argument, NULL, 'r'},
	{"somig-ctl-port", required_argument, NULL, 'e'},
	{"Yell-periodically", required_argument, NULL, 'Y'},
#endif
	{"debug-payload", no_argument, NULL, 'x'},
...
/*
 * 146
 * b:c:df:hi:k:l:n:p:st:uvw:
 * A:B:C:DF:I:JL:M:NO:P:RS:T:VX:Z
 *
 * m: SOMIG mode 
 * a: PrimaryAddr
 * r: ReplicaAddr
 ...
 *
 */
    while ((flag = getopt_long(argc, argv, "K:EUY:m:a:r:e:p:f:i:D1VJvsc:ub:t:n:k:l:P:Rw:B:M:N46S:L:ZO:F:A:T:C:dI:hxX:", longopts, NULL)) != -1) {
        switch (flag) {
#ifdef SOMIGRATION
	    case 'E':
		test->somig_execute_kernel = 1;
		break;
	    case 'U':
		iperf_set_test_repeating_payload(test, 1);
		break;
	    case 'K':
		test->somig_migration = atoi(optarg);
		break;
	    case 'm':
		somig_flag = optarg;
		if (strcmp("mso", optarg) == 0) {
		    test->somig_appmode = PRIMARY;
		} else if (strcmp("rso", optarg) == 0) {
		    test->somig_appmode = REPLICA;
		} else if (strcmp("cli", optarg) == 0) {
		    test->somig_appmode = CLIENT;
		} else {
		    printf("Invalid SOMIG operation mode. See usage[-h].\n");
		    return -1;
		}
		break;
	    case 'a':
		test->somig_primary_hostname = strdup(optarg);
		break;
	    case 'r':
		test->somig_replica_hostname = strdup(optarg);
		break;
	    case 'e':
		portno = atoi(optarg);
...
#ifndef SMCP
                test->somig_ctl_port = portno;
#endif
		break;
	    case 'Y':
		test->yell_period = atoi(optarg);
		break;
#endif
	    case 'x':
		iperf_set_test_repeating_payload(test, 1);
		test->debug_payload = 1;
		break;
            case 'p':
		portno = atoi(optarg);
		if (portno < 1 || portno > 65535) {
...
#ifdef SOMIGRATION
		printf("With SOMIG enabled\n");
#endif
#ifdef SMCP
		printf("With SMCP enabled\n");
#endif
                exit(0);
            case 's':
                if (test->role == 'c') {
...
iperf_send(struct iperf_test *test, fd_set *write_setP)
{
    ...
    static int send_seed = 0;
    SLIST_FOREACH(sp, &test->streams, streams) {
	somig_update_debug_payload(sp->buffer, test->settings->blksize, sp->id, ++send_seed);
    }
...
iperf_defaults(struct iperf_test *testp)
{
    ...
    testp->debug_payload = 0;
#ifdef SOMIGRATION
    testp->somig_appmode = SOMIG_NONE;
    testp->somig_primary_hostname = strdup(SOMIG_PRIMARY_HOSTNAME);
    testp->somig_replica_hostname = strdup(SOMIG_REPLICA_HOSTNAME);
    testp->somig_ctl_port = SOMIG_CTL_PORT;
    testp->somig_appmode = SOMIG_MODE;
    testp->somig_migration = 0;
    testp->somig_execute_kernel = 0;
    testp->yell_period = 0;
...
#ifdef SOMIGRATION
    if (test->somig_primary_hostname)
	free(test->somig_primary_hostname);
    if (test->somig_replica_hostname)
	free(test->somig_replica_hostname);
#endif
...
print_interval_results(struct iperf_test *test, struct iperf_stream *sp, cJSON *json_interval_streams)
{
	...
	if (test->sender_has_retransmits == 1 && sp->sender) {
		...
		gettimeofday(&tn, NULL);
#ifdef SOMIGRATION
		int cli_cpu, smg_cpu, iperf_cpu, print = 1;
		int cli_cpu_usage, smg_cpu_usage, iperf_cpu_usage;
		uint64_t mbuf_usg, mbuf_9k_usg, mem_size;
		struct tcp_info tminfo_mso, tminfo_cso;
		socklen_t ti_len = sizeof(struct tcp_info);
		if (getsockopt(sp->socket, IPPROTO_TCP, TCP_INFO, (void *)&tminfo_mso, &ti_len)) {
			perror("getsockopt - TCP_CTLSOINFO CTLSO");
			print = 0;
		}

		if (print == 1) {
		    cli_cpu = tminfo_mso.tcpi_cli_cpu_id;
		    smg_cpu = tminfo_mso.tcpi_smg_cpu_id;
		    iperf_cpu = getpid();
		    cli_cpu_usage = cli_cpu;
		    smg_cpu_usage = smg_cpu;
		    iperf_cpu_usage = iperf_cpu;

		    somig_stat_get_cpu_usage(&cli_cpu_usage, &smg_cpu_usage, &iperf_cpu, &iperf_cpu_usage); 
		    //somig_stat_get_mbuf_usage(&mbuf_usg, &mbuf_9k_usg);
		    somig_stat_get_net_memory_usage(&mem_size);
		    mbuf_usg = 0;
		    mbuf_9k_usg = 0;
		    cJSON_AddItemToArray(json_interval_streams, 
			iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  retransmits: %d  snd_cwnd:  %d  rtt:  %d  rttvar: %d  pmtu: %d  omitted: %b sender: %b cli_cpu_id: %d cli_cpu_usage: %d smg_cpu_id: %d smg_cpu_usage: %d iperf_cpu_id: %d iperf_cpu_usage: %d rtt: %d mbuf_count: %d mbuf_9k_count: %d mem_size: %d", 
			    (int64_t) sp->socket, (double) st, (double) et,
			    (double) irp->interval_duration, 
			    (int64_t) irp->bytes_transferred, bandwidth * 8, 
			    (int64_t) irp->interval_retrans, 
			    (int64_t) irp->snd_cwnd, 
			    (int64_t) irp->rtt, 
			    (int64_t) irp->rttvar, 
			    (int64_t) irp->pmtu, 
			    irp->omitted, 
			    sp->sender,
			    (int64_t)cli_cpu, (int64_t)cli_cpu_usage,
			    (int64_t)smg_cpu, (int64_t)smg_cpu_usage,
			    (int64_t)iperf_cpu, (int64_t)iperf_cpu_usage,
			    (int64_t)tminfo_mso.tcpi_rtt,
			    (int64_t)mbuf_usg,
			    (int64_t)mbuf_9k_usg,
			    (int64_t)mem_size
		));
...
#else
		cJSON_AddItemToArray(json_interval_streams, iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  retransmits: %d  snd_cwnd:  %d  rtt:  %d  rttvar: %d  pmtu: %d  omitted: %b sender: %b accu_sec: %d accu_usec %d", (int64_t) sp->socket, (double) st, (double) et, (double) irp->interval_duration, (int64_t) irp->bytes_transferred, bandwidth * 8, (int64_t) irp->interval_retrans, (int64_t) irp->snd_cwnd, (int64_t) irp->rtt, (int64_t) irp->rttvar, (int64_t) irp->pmtu, irp->omitted, sp->sender, tn.tv_sec, tn.tv_usec));
#endif
	    }
	    else {
...
/*
		...
		socklen_t ti_len = sizeof(struct tcp_info);
		if (getsockopt(sp->socket, IPPROTO_TCP, TCP_INFO, (void *)&tinfo, &ti_len)) {
			perror("getsockopt - TCP_INFO");
		}
		iperf_printf(test, report_bw_format2, tinfo.tcpi_snd_cwnd,
			tinfo.tcpi_snd_ssthresh, tinfo.tcpi_rtt, 
			tinfo.tcpi_rttvar, tinfo.tcpi_snd_rexmitpack, tinfo.tcpi_rto);
*/
//#endif
	    }
	} else {
		...
		if (print == 1) {
		    ...
		    mbuf_9k_usg = 0;
		    cJSON_AddItemToArray(json_interval_streams, 
			iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  omitted: %b sender: %b cli_cpu_id: %d cli_cpu_usage: %d smg_cpu_id: %d smg_cpu_usage: %d iperf_cpu_id: %d iperf_cpu_usage: %d rtt: %d mbuf_count: %d mbuf_9k_count: %d mem_size: %d", 
			(int64_t) sp->socket, (double) st, (double) et, 
			(double) irp->interval_duration, 
			(int64_t) irp->bytes_transferred, bandwidth * 8, 
...
#else
		cJSON_AddItemToArray(json_interval_streams, iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  omitted: %b sender: %b", 
		    (int64_t) sp->socket, 
		    (double) st, (double) et, 
		    (double) irp->interval_duration,
...
#if 0
#ifdef SOMIGRATION
		if (test->somig_appmode == PRIMARY || test->somig_appmode == REPLICA) {
		    struct tcp_info tminfo_mso, tminfo_cso;
		    socklen_t ti_len = sizeof(struct tcp_info);
		    if (getsockopt(sp->socket, IPPROTO_TCP, TCP_INFO, (void *)&tminfo_mso, &ti_len)) {
			perror("getsockopt - TCP_CTLSOINFO MSO");
		    }
		    if (test->somig_appmode == PRIMARY) {
			if (getsockopt(sp->socket, IPPROTO_TCP, TCP_CTLSOINFO, (void *)&tminfo_cso, &ti_len)) {
			    perror("getsockopt - TCP_CTLSOINFO CTLSO");
			}
		    }
		    //socklen_t sl = sizeof(int);
		    //getsockopt(sp->socket, SOL_SOCKET, SO_MIG_DEBUG, &ti_len, &sl);
		    iperf_printtcpinfo(test, &tminfo_mso, 
			(test->somig_appmode == PRIMARY ? (&tminfo_cso) : (NULL)));
		}
	   }
#else
		struct tcp_info tinfo;
		...
		iperf_printf(test, report_bw_format2, sp->socket, tinfo.tcpi_snd_cwnd, 
			tinfo.tcpi_rtt, tinfo.tcpi_snd_rexmitpack, tinfo.tcpi_rcv_ooopack);
	    }
#endif
#endif
	    }
...
#ifdef SOMIGRATION
/*
 *[P]cw%u rtt%uus rexmtpk%u ooo%u [C]cw%u rtt%uus rexmtpk%u ooo%u\n"
 */
int
iperf_printtcpinfo(struct iperf_test *test, struct tcp_info *i1, struct tcp_info *i2)
{
    if (i2 != NULL)
	iperf_printf(test, report_bw_format_somig_mso, i1->tcpi_snd_cwnd, i1->tcpi_rtt, 
	    i1->tcpi_snd_rexmitpack, i1->tcpi_rcv_ooopack, i1->tcpi_smg_bufsize,
	    i2->tcpi_snd_cwnd, i2->tcpi_snd_rexmitpack, i2->tcpi_rtt, 
	    i2->tcpi_rcv_ooopack, i2->tcpi_smg_bufsize);
    else
	iperf_printf(test, report_bw_format_somig_rso, i1->tcpi_snd_cwnd, i1->tcpi_rtt, 
	    i1->tcpi_snd_rexmitpack, i1->tcpi_rcv_ooopack, i1->tcpi_smg_bufsize);
}
#endif
