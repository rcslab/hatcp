...
iperf_tcp_accept(struct iperf_test * test)
{
	...
	int	opt = 1; /* Default ID for the first CtlSo */
	if (setsockopt(s, IPPROTO_TCP, TCP_CTLSOINFO, &opt, sizeof(opt))) {
	    perror("setsockopt - TCP_CTLSOINFO");
	    return (-1);
	}
#ifdef SOMIG_TIMING_DIAG
        if (setsockopt(s, SOL_SOCKET, SO_MIG_TIMING_DIAG, &opt, sizeof opt)) {
            perror("setsockopt - SOMIG_TIMING_DIAG");
            return (-1);
        }
#endif
    }
