...
{
	return netannounce_somig(domain, proto, local, bind_dev, port, 0, NULL, NULL, 0);
}
#endif

int
#ifdef SOMIGRATION
netannounce_somig(int domain, int proto, const char *local, const char *bind_dev, int port,
	int mode, const char *mso, const char *rso, int ctl_port)
#else
netannounce(int domain, int proto, const char *local, const char *bind_dev, int port)
#endif
{
...
#define RSO  1
    if (mode == RSO) {
#ifdef SMCP
	if ((s = somig_init_replica(domain, AF_INET, mso, rso, local, port)) < 0) {
#else
	if ((s = somig_init_replica(domain, AF_INET, mso, rso, ctl_port)) < 0) {
#endif
		printf("Cannot initialize replica\n");
		return -2;
	}
/*
	if (somig_init_app_sock(s, local, ctl_port) != 0) {
		printf("Cannot initialize replica so\n");
		return -1;
	}
*/
#undef RSO
...
#define MSO 2
    if (mode == MSO) {
#ifdef SMCP
	if (somig_init_primary(s, domain, AF_INET, mso, ctl_port, local, port) < 0) {
#else
	if (somig_init_primary(s, domain, AF_INET, mso, ctl_port) < 0) {
#endif
		printf("Cannot initialize primary\n");
		return -2;
	}
    }
